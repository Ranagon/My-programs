...
namespace Szakdolgozat3// Ebbe a formban a "FindStringExact" függvénnyel volt problémám, nem találta a kifejezéseket a comboboxban. Most működik, de lehetséges a kódhalmozás. 
{
    public partial class UjPartnerFrm : Form
    {
        ...
        {
            if (Postacim_chbox.Checked && SzamlaIrsz_txb.Text != "Kötelező kitölteni!" && SzamlaTelepules_txb.Text != "Kötelező kitölteni!" &&
                SzamlaKozeruletNeve_txb.Text != "Kötelező kitölteni!" && SzamlaHazSzam_txb.Text != "Kötelező kitölteni!")
            {
        ...
        private void TelefonSzam_txb_KeyPress(object sender, KeyPressEventArgs e)
        {
            KivetelKezeles.StringLetiltoDecimal(SzamlaIrsz_txb, e);
        }

        private void Fax_txb_KeyPress(object sender, KeyPressEventArgs e)
        {
            KivetelKezeles.StringLetiltoDecimal(SzamlaIrsz_txb, e);
        }
    }
